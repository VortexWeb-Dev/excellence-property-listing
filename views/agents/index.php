...
<script>
    ...
    async function fetchAgents(page = 1) {
        ...
        const apiUrl = `${baseUrl}/crm.item.list?entityTypeId=${entityTypeId}&order[id]=desc&select[0]=id&select[1]=ufCrm8AgentName&select[2]=ufCrm8AgentEmail&select[3]=ufCrm8AgentMobile&select[4]=ufCrm8AgentLicense&select[5]=ufCrm8AgentPhoto&select[6]=ufCrm8AgentId&start=${(page - 1) * pageSize}`;
        ...
        try {
            ...
            agentList.innerHTML = agents
                .map(
                    (agent) => `
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm8AgentId}</td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm8AgentEmail || ''}</td>
