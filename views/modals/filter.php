...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const fetchAgents = async () => {
            try {
                const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${AGENTS_ENTITY_ID}&select[0]=id&select[1]=ufCrm8AgentName&order[ufCrm8AgentName]=asc`);
        ...
        const fetchDevelopers = async () => {
            try {
                const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${DEVELOPERS_ENTITY_ID}&select[0]=id&select[1]=ufCrm14DeveloperName&order[ufCrm14DeveloperName]=asc`);
                const data = await response.json();
                const totalDevelopers = data.total;
                const developers = [];

                for (let i = 0; i < Math.ceil(totalDevelopers / 50); i++) {
                    const paginatedResponse = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${DEVELOPERS_ENTITY_ID}&select[0]=id&select[1]=ufCrm14DeveloperName&order[ufCrm14DeveloperName]=asc&start=${i * 50}`);
        ...
        const fetchAndDisplayOptions = async () => {
            try {
                ...
                createSelectOptions(developers, developerSelect, 'ufCrm14DeveloperName', 'ufCrm14DeveloperName');
